...
from .util.bounded_kde import bounded_gaussian_kde
...
def idca_statistics_data(
    var, bw_method="scott", choice=None, avail=None, wgt=None, scale_ch=True
):
    ...
    if choice is not None:
        kde_ch = {}
        for k, v in data.items():
            y = bounded_gaussian_kde(v, bw_method=bandwidth, weights=data_ch[k] * wgt)(
                x
            )
            if scale_ch:
            ...
            kde_ch[k] = y
        plot_data = pd.concat(
            [pd.DataFrame(kde), pd.DataFrame(kde_ch)],
            axis=1,
            keys=["Density", "Chosen"],
        )
        plot_data = (
...
def idca_statistics(
    ...
    stats, plot_data = idca_statistics_data(
        var, bw_method=bw_method, choice=choice, avail=avail, wgt=wgt, scale_ch=scale_ch
    )
    name = var.name or "Variable"
