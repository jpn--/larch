...
class OMX(_omx_base_class):
	...
	def __repr__(self):
		...
		for i in sorted(self.lookup._v_children.keys()):
			s += "\n |    {0:{3}s} ({1} {2})".format(i, self.lookup._v_children[i].shape[0],
			                                         self.lookup._v_children[i].dtype, just)
		return s
	...
	def crop(self, shape0, shape1=None, *, complevel=1, complib='zlib'):
		...
		for i in self.data._v_children:
			self.add_matrix(i, self.data._v_children[i][:shape0, :shape1], overwrite=True, complevel=complevel,
			                complib=complib, ignore_shape=True)
		for i in self.lookup._v_children:
			if self.lookup[i].shape[0] == start0:
				self.add_lookup(i, self.lookup._v_children[i][:shape0], overwrite=True, complevel=complevel,
				                complib=complib, ignore_shape=True)
			elif self.lookup[i].shape[1] == start1:
				self.add_lookup(i, self.lookup._v_children[i][:shape1], overwrite=True, complevel=complevel,
				                complib=complib, ignore_shape=True)
		shp = numpy.empty(2, dtype=int)
		shp[0] = shape0
		shp[1] = shape1
			...
			if shape != self.shape[0] and shape != self.shape[1]:
				if self.shape[0] == 0 and self.shape[1] == 0:
					...
					raise OMXIncompatibleShape(
						'this omx has shape {!s} but you want to set {!s}'.format(self.shape, shape))
		else:
			if self.shape[0] != self.shape[1]:
				raise OMXIncompatibleShape('this omx has shape {!s} but you did not pick one'.format(self.shape))
			shape = self.shape[0]
		return self.create_carray(self.lookup, name, atom=atom, shape=numpy.atleast_1d(shape),
	...
	def add_blank_matrix(self, name, atom=None, shape=None, complevel=1, complib='zlib', **kwargs):
		...
		if shape is not None:
			if shape[:2] != self.shape:
				raise OMXIncompatibleShape(
					'this omx has shape {!s} but you want to set {!s}'.format(self.shape, shape[:2]))
		else:
			shape = self.shape
		return self.create_carray(self.data, name, atom=atom, shape=shape,
		                          filters=_tb.Filters(complib=complib, complevel=complevel), **kwargs)
		...
		if not ignore_shape:
			...
			if self.data._v_nchildren > 0:
				if obj.shape != self.shape:
					raise OMXIncompatibleShape(
						'this omx has shape {!s} but you want to add {!s}'.format(self.shape, obj.shape))
		if self.data._v_nchildren == 0:
			shp = numpy.empty(2, dtype=int)
			shp[0] = obj.shape[0]
			shp[1] = obj.shape[1]
		...
		if not ignore_shape:
			...
			if self.data._v_nchildren > 0:
				if obj.shape[0] not in self.shape:
					raise OMXIncompatibleShape(
						'this omx has shape {!s} but you want to add a lookup with {!s}'.format(self.shape, obj.shape))
		...
		if matrix:
			if name in self.data._v_children:
				...
				if require_smaller and neww.size_on_disk >= orig.size_on_disk:
					warnings.warn(
						"abort change_atom_type on {}, {} > {}".format(name, neww.size_on_disk, orig.size_on_disk))
					neww._f_remove()
				else:
	...
	def import_datatable_as_lookups(self, filepath, chunksize=10000, column_map=None, log=None, n_rows=None,
	                                zone_ix=None, zone_ix1=1, drop_zone=None):
	...
	def import_datatable(self, filepath, one_based=True, chunksize=10000, column_map=None, default_atom='float32',
	                     log=None):
	...
	@classmethod
	def change_cols(cls, newfile, otherfile, datanames, lookuprownames, lookupcolnames, newshape, newslicecols=None,
	                oldslicecols=None):
		"""
		...
		for name in datanames:
			n = self.add_blank_matrix(name, atom=otherfile.data._v_children[name].atom, shape=newshape, complevel=1,
			                          complib='zlib')
	...
	def info(self):
		...
		## Header
		a = ART(columns=('TABLE', 'DTYPE'), n_head_rows=1,
		        title="OMX ({},{}) @ {}".format(self.shape[0], self.shape[1], self.filename), short_title=None)
