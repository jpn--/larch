...
//	sql << "Generated by sim_conditional_probability at " << etk::current_datetime();
//	SQLiteStmtPtr z = _Data->sql_statement("INSERT INTO elm_tables VALUES (?,?,?,?,?,?);");
//	z->bind_text(1, tablename); // tablename
//	z->bind_text(2, "caseid");  // selfkey
//	z->bind_text(3, "caseid");  // masterkey
//	z->bind_text(4, "idCO");    // kind
//	z->bind_text(5, "");        // altfield
//	z->bind_text(6, sql.str()); // notes
...
//		_Data->sql_statement("DELETE FROM elm_tables_temp WHERE tablename=?")->bind_text(1, tablename)->execute_until_done();
	...
	// Insert the new table into elm_tables
//	sql << "Generated by simulate_probability at " << etk::current_datetime();
//	SQLiteStmtPtr z = _Data->sql_statement("INSERT INTO elm_tables VALUES (?,?,?,?,?,?);");
//	z->bind_text(1, tablename); // tablename
//	z->bind_text(2, "caseid");  // selfkey
//	z->bind_text(3, "caseid");  // masterkey
 ...
 const double&    freedom_multiplier)
{
	...
	if (_Data) {
		...
		_Data->check_co(column_name);
		if (!alt_name.empty()) {
			slot = _Data->DataDNA()->slot_from_name(alt_name);
		} else {
			if (alt_code==cellcode_empty) {
				OOPS("utility.co input requires that you specify an alternative.");
			}
			slot = _Data->DataDNA()->slot_from_code(alt_code);
		}
	}
...
string elm::Model2::prints(const unsigned& precision, const unsigned& cell_width) const
{
	...
	ret << "Model Network\n";
	LINER(ret,'-');
	ret << ComponentGraphDNA(&Input_LogSum,&Input_Edges,_Data).__repr__() << "\n";
...
/*	if (_is_setUp) {
		_TallyChosen = tally_chosen();
		_TallyAvail = tally_avail();
	}
	
	if (_TallyChosen && _TallyAvail && _Data) {
		LINER(ret,'=');
		ret << "Data Summary\n";
		LINER(ret,'-');
		
		ret << std::setw(cell_width) << std::left << "Alt" << std::setw(cell_width) << "# Chosen" << std::setw(cell_width) << "# Avail" << "\n" << std::right;
		for ( unsigned a=0; a<_TallyChosen->size1(); a++) {
			ret << std::setw(cell_width) << _Data->alternative_names()[a] << std::setw(cell_width) << _TallyChosen->operator()(a) << std::setw(cell_width) << _TallyAvail->operator()(a) << "\n";
		}
	} else if (_TallyChosen) {
		LINER(ret,'=');
		ret << "Data Summary\n";
		LINER(ret,'-');
		
		ret << std::setw(cell_width) << std::left << "Alt" << std::setw(cell_width) << "# Chosen" << "\n" << std::right;
		for ( unsigned a=0; a<_TallyChosen->size1(); a++) {
			ret << std::setw(cell_width) << _Data->alternative_names()[a] << std::setw(cell_width) << _TallyChosen->operator()(a) << "\n";
		}
	}
